...
$sql = "SELECT chat_whatsapp FROM vendor_details where ids = '6'";
//echo $sql;
$result = mysqli_query($con, $sql);

$phone = "";
if ($row = mysqli_fetch_assoc($result)) {
    $phone = preg_replace('/[^0-9]/', '', $row['chat_whatsapp']); // clean number
    ...
    <style>
        ...
        /*}*/
         ...
         @font-face {
                ...
                font-style: normal;
                font-display: fallback;
                src: url('../../assets/sands/nunito_n4.fc49103dc396b42cae9460289072d384b6c6eb63.woff2') format('woff2'),
                     url('../../assets/sands/nunito_n4.5d26d13beeac3116db2479e64986cdeea4c8fbdd.woff') format('woff');
            }
        ...
        /*}*/
          ...
          @font-face {
                ...
                font-style: italic;
                font-display: fallback;
                src: url('../../assets/sands/nunito_i4.fd53bf99043ab6c570187ed42d1b49192135de96.woff2') format('woff2'),
                     url('../../assets/sands/nunito_i4.cb3876a003a73aaae5363bb3e3e99d45ec598cc6.woff') format('woff');
            }
        ...
        /*}*/
         ...
         @font-face {
                ...
                font-style: normal;
                font-display: fallback;
                src: url('../../assets/sands/nunito_n7.37cf9b8cf43b3322f7e6e13ad2aad62ab5dc9109.woff2') format('woff2'),
                     url('../../assets/sands/nunito_n7.45cfcfadc6630011252d54d5f5a2c7c98f60d5de.woff') format('woff');
            }
        ...
        /* Typography (body) */
            ...
            @font-face {
                ...
                font-style: italic;
                font-display: fallback;
                src: url('../../assets/sands/nunito_i7.3f8ba2027bc9ceb1b1764ecab15bae73f86c4632.woff2') format('woff2'),
                     url('../../assets/sands/nunito_i7.82bfb5f86ec77ada3c9f660da22064c2e46e1469.woff') format('woff');
            }
        ...
        box-shadow: 0 0 0 2px rgb(245 235 174), 0 0 15px rgb(255 239 165);
           ...
           .dz-media {
            ...
            .dz-media video {
                width: 100%;
                height: 100%;
                object-fit: cover; /* 🔥 THIS CROPS VIDEO */
            }
            ...
            .product-gallery-loader .loader-content {
                text-align: center;
            }
...
<body id="bg">
    ...
    $(document).ready(function() {
        ...
        let productId = null;
        let platform = null; // Add platform variable
    
        const urlParams = new URLSearchParams(window.location.search);
        let productIds = urlParams.get('id');
        
        console.log("Incoming ID:", productIds);
        
        // ✅ Detect automatically
        if (!isEncrypted(productIds)) {
            // Normal ID
            productId = productIds;
            console.log("Normal ID flow:", productId);
            loadProductDetails(productId);
        } else {
            // Encrypted flow
            fetch('Link?id=' + encodeURIComponent(productIds))
                .then(response => response.json())
                .then(data => {
                    productId = data.productId;
                    platform = data.platform; // Capture platform from response
                    
                    console.log("Encrypted ID flow:", productId);
                    console.log("Platform:", platform);
                    
                    // Store platform in session or data attribute for later use if needed
                    if (platform) {
                        sessionStorage.setItem('referral_platform', platform);
                        // Or store in data attribute
                        $('body').data('referral-platform', platform);
                    }
        ...
        function isEncrypted(value) {
            return !/^\d+$/.test(value); // true if NOT only digits
        }
        ...
        //loadProductDetails(productId);
        function loadImagesWithSpinner(imageElements, callback) {
            let loadedCount = 0;
        ...
        function loadSizeChartData() {
            $('#sizeChartLoader').show();
            $('.size-chart-main').addClass('loading');
        
            $.ajax({
                url: 'controller/product_details_controller.php',
                type: 'POST',
                data: {
                    action: 'size_chart_details',
                    product_id: productId
                },
                dataType: 'json',
                success: function(response) {
                    $('#sizeChartLoader').hide();
                    $('.size-chart-main').removeClass('loading');
        
                    if (response.status === 'success' && response.data.size_chart) {
                        renderSizeChart(response.data);
                    }
                },
                error: function() {
                    $('#sizeChartLoader').hide();
        ...
        function renderSizeChart(product) {
            ...
            const measurements = product.size_chart_measurements || [];
            const heightLength = product.size_chart_height_length || [];
            const tips = product.size_chart_tips || [];
            const isArabic = window.currentLanguage === 'arabic';
            
            // Update sidebar header
            $('.size-chart-header h3').text(sizeChart.chart_name || (isArabic ? 'دليل مقاسات العباية' : 'ABAYA SIZE GUIDE'));
            
            // Update descriptions from size_charts table - FIXED SELECTORS
            if (sizeChart.description_en || sizeChart.description_ar) {
                // Update English description - using .size-chart-description-en class
               
                if (sizeChart.description_en) {
                    $('.size-chart-description-en').html(sizeChart.description_en);
                }
                
                // Update Arabic description - using .size-chart-description-ar class
                if (sizeChart.description_ar) {
                    $('.size-chart-description-ar').html(sizeChart.description_ar);
                }
            } else {
                // Default descriptions if none exist
                $('.size-chart-description-en').html(
                    '<p>Standard Size Abayas. These sizes are based on our experience and are the best fit for most customers. However, if you prefer your abaya a little longer or shorter, you can always choose the size that feels right for you.</p>'
                );
                $('.size-chart-description-ar').html(
                    '<p>دليل المقاسات (توصية) هذه المقاسات موصى بها بناءً على خبرتنا، وتناسب أغلب العملاء. لكن إذا كنتِ تفضلين العباءة أطول أو أقصر قليلًا، يمكنكِ اختيار المقاس الذي يناسب راحتك.</p>'
                );
            }
            
            // Render measurements table - FIXED SELECTOR
                ...
                measurements.forEach(item => {
                    measurementsHtml += `
                        <tr>
                            ...
                            <td>${item.chest_inch}</td>
                            <td>${item.shoulder_inch}</td>
                        </tr>
                    `;
                });
                
                $('.size-chart-measurements-body').html(measurementsHtml);
            } else {
                // Default measurements if none in database
                $('.size-chart-measurements-body').html(`
                    <tr><td>S</td><td>20</td><td>14.5</td></tr>
                    <tr><td>M</td><td>22</td><td>15</td></tr>
                    <tr><td>L</td><td>24</td><td>16</td></tr>
                    <tr><td>XL</td><td>26</td><td>16.5</td></tr>
                `);
            }
            
            // Render height-length table - FIXED SELECTOR
            if (heightLength.length > 0) {
                let heightLengthHtml = '';
                heightLength.forEach(item => {
                    heightLengthHtml += `
                        <tr>
                            <td>${item.abaya_length_inch}</td>
                            <td>${item.height_cm}</td>
                        </tr>
                    `;
                });
                
                $('.size-chart-height-length-body').html(heightLengthHtml);
            } else {
                // Default height-length mappings if none in database
                const defaultMappings = [
                    {abaya_length_inch: 49, height_cm: 150},
                    {abaya_length_inch: 50, height_cm: 151},
                    {abaya_length_inch: 50, height_cm: 152},
                    {abaya_length_inch: 51, height_cm: 153},
                    {abaya_length_inch: 51, height_cm: 154},
                    {abaya_length_inch: 52, height_cm: 155},
                    {abaya_length_inch: 52, height_cm: 156},
                    {abaya_length_inch: 52, height_cm: 157},
                    {abaya_length_inch: 53, height_cm: 158},
                    {abaya_length_inch: 53, height_cm: 159},
                    {abaya_length_inch: 54, height_cm: 160},
                    {abaya_length_inch: 54, height_cm: 161},
                    {abaya_length_inch: 54, height_cm: 162},
                    {abaya_length_inch: 54, height_cm: 163},
                    {abaya_length_inch: 55, height_cm: 164},
                    {abaya_length_inch: 55, height_cm: 165},
                    {abaya_length_inch: 55, height_cm: 166},
                    {abaya_length_inch: 55, height_cm: 167},
                    {abaya_length_inch: 56, height_cm: 168},
                    {abaya_length_inch: 56, height_cm: 169},
                    {abaya_length_inch: 56, height_cm: 170},
                    {abaya_length_inch: 56, height_cm: 171},
                    {abaya_length_inch: 57, height_cm: 172},
                    {abaya_length_inch: 57, height_cm: 173},
                    {abaya_length_inch: 57, height_cm: 174},
                    {abaya_length_inch: 58, height_cm: 175},
                    {abaya_length_inch: 58, height_cm: 176},
                    {abaya_length_inch: 60, height_cm: 177},
                    {abaya_length_inch: 60, height_cm: 178},
                    {abaya_length_inch: 60, height_cm: 179},
                    {abaya_length_inch: 60, height_cm: 180}
                ];
                
                let defaultHtml = '';
                defaultMappings.forEach(item => {
                    defaultHtml += `<tr><td>${item.abaya_length_inch}</td><td>${item.height_cm}</td></tr>`;
                });
                $('.size-chart-height-length-body').html(defaultHtml);
            }
            
            // Render measurement tips if available - FIXED SELECTORS
            if (tips && tips.length > 0) {
                let englishTips = tips.filter(tip => tip.language === 'en' || !tip.language);
                let arabicTips = tips.filter(tip => tip.language === 'ar');
                
                if (englishTips.length > 0) {
                    let tipsHtml = '';
                    englishTips.forEach(tip => {
                        tipsHtml += `<p class="small mb-1">• ${tip.tip_text}</p>`;
                    });
                    $('.size-chart-tips-en').html(tipsHtml);
                }
                
                if (arabicTips.length > 0) {
                    let tipsArabicHtml = '';
                    arabicTips.forEach(tip => {
                        tipsArabicHtml += `<p class="small mb-1">• ${tip.tip_text}</p>`;
                    });
                    $('.size-chart-tips-ar').html(tipsArabicHtml);
                }
            }
            
            // Add CSS for Quill content (images and formatting)
            addSizeChartStyles();
        }
        function addSizeChartStyles() {
            // Check if styles already added
            if ($('#size-chart-styles').length) return;
            
            const style = `
                <style id="size-chart-styles">
                    .size-chart-description-en img,
                    .size-chart-description-ar img {
                        max-width: 100%;
                        height: auto;
                        display: block;
                        margin: 15px auto;
                    ...
                    .size-chart-description-en p,
                    .size-chart-description-ar p {
                        margin-bottom: 15px;
                        line-height: 1.6;
                    }
                    
                    .size-chart-description-en h1,
                    .size-chart-description-en h2,
                    .size-chart-description-en h3,
                    .size-chart-description-en h4,
                    .size-chart-description-ar h1,
                    .size-chart-description-ar h2,
                    .size-chart-description-ar h3,
                    .size-chart-description-ar h4 {
                        margin: 20px 0 10px;
                        color: #333;
                    }
                    
                    .size-chart-description-en ul,
                    .size-chart-description-en ol,
                    .size-chart-description-ar ul,
                    .size-chart-description-ar ol {
                        padding-left: 20px;
                        margin-bottom: 15px;
                    }
                    ...
                    .size-chart-description-en a:hover,
                    .size-chart-description-ar a:hover {
                        color: #000;
                    }
                    
                    /* Handle broken images */
                    .size-chart-description-en img[src=""],
                    .size-chart-description-ar img[src=""],
                    .size-chart-description-en img:not([src]),
                    .size-chart-description-ar img:not([src]) {
                        display: none;
                    }
        ...
        $(document).on('error', '.size-chart-description-en img, .size-chart-description-ar img', function() {
            $(this).replaceWith('<p class="text-danger small">[Image failed to load]</p>');
        });
        // Update your loadProductDetails function
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        // Render size chart with the data - FIXED: Check if size_chart exists
                        // if (response.data) {
                        //     renderSizeChart(response.data);
                        // }
                        
                        // Add image error handling after content is loaded
                        setTimeout(function() {
                            $('.size-chart-description-en img, .size-chart-description-ar img').on('error', function() {
                                $(this).replaceWith('<p class="text-danger small">[Image failed to load]</p>');
                            });
                        }, 500);
                    } else {
        ...
        function renderProductDetails(product) {
            ...
            if (product.category_name && product.category_id) {
                $('#category-link')
                    .text(product.category_name)
                    .attr('href', 'https://darjanafashion.com/Shop?cat_id=' + product.category_id);
            }
        ...
        function updateImageGallery(images, product_video, productName, baseUrl, baseVideoUrl, defaultImage) {
            ...
            if (product_video) {
                const videoUrl = baseVideoUrl + product_video;
                imageSlides += `
                    <div class="swiper-slide">
                        <div class="dz-media DZoomImage">
                            <video 
            ...
            // Track loading of all images and videos
            const allMedia = $('#lightgallery2 img, #lightgallery2 video, .thumb-swiper-lg img, .thumb-swiper-lg video');
            let loadedCount = 0;
