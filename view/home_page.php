...
.about-box .about-img img {
    width: 100%;
    height: 350px !important;  /* Increased from 280px to 350px */
    object-fit: cover;
    transition: transform 0.3s ease;
}

.about-box .about-img img:hover {
    transform: scale(1.05);
}

.about-box .about-btn .btn {
    background: #fff;
...
@media (max-width: 992px) {
    .about-box .about-img img {
        height: 300px !important;  /* Slightly smaller on tablets */
    }
}
...
//          console.log('Language button exists?', $('#languageSwitcher').length > 0);
				...
				success: function(response) {
					if (response.data && response.data.length > 0) {
						...
						response.data.forEach((item, index) => {
							...
							let imagePath = `https://admin.darjanafashion.com/assets/img/categories/${item.category_image}`;
        ...
        function renderFeaturedProducts() {
            ...
            window.featuredProducts.forEach((product, index) => {
                ...
                // If offer price exists use it,
                // otherwise use selling price
                const baseOfferPrice =
                    parseFloat(product.amount_offer || product.off_amt) ||
                    parseFloat(product.amount_selling || product.selling_amt) ||
                    baseMrpPrice;
                ...
                // productHTML += `
                //     <li class="card-container col-6 col-md-6 col-lg-6 col-xl-6 ${product.category_type} wow fadeInUp" data-wow-delay="${delayStr}">
                //         <div class="shop-card">
                //         <div class="img-gap">
                //             <div class="dz-media">
                ...
                //                         <a href="ProductDetails?id=${product.ids}" class="product-title">${displayName}</a>
                //                     </h7>
                //                 </div>
                //                 <div class="row">
                //                     <div class="price">
					...
					if (response.data && response.data.length > 0) {
						...
						for (let i = 0; i < response.data.length; i += 2) {
							...
							const getSlideBlock = (item) => {
								if (!item) return '';
								const imageUrl = `https://admin.darjanafashion.com/assets/img/categories/${item.category_image || 'default.jpg'}`;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.data && response.data.length > 0) {
                        ...
                        // Add categories as individual slides
                        response.data.slice(0, 12).forEach(function(item) {
                            const imageUrl = `https://admin.darjanafashion.com/assets/img/categories/${item.category_image || 'default.jpg'}`;
                            const category = item.category_type;
                                ...
                                <div class="swiper-slide">
                                    <div class="about-box text-center">
                                        <div class="about-img mb-2">
                                            <img src="${imageUrl}" alt="${category}" style="width:100%;height:350px;object-fit:cover;border-radius:10px;">
                                        </div>
                                        <div class="about-btn">
                                            <a class="btn btn-white btn-md" href="Shop?cat_id=${item.ids}" style="padding: 10px 25px; font-size: 16px;">${category}</a>
                                        </div>
                                    </div>
                                </div>
                            `;
                        ...
                        // Initialize Swiper with 4 slides per view on desktop
                        new Swiper('.about-swiper', {
                            ...
                            breakpoints: {
                                ...
                                992: { 
                                    slidesPerView: 4,  // 4 categories per slide on desktop
                                    spaceBetween: 30
                                }
                            }
                        });
				...
				success: function(response) {
					if (response.data && response.data.length > 0) {
						...
						response.data.forEach(function(collection, idx) {
							const imageUrl = `https://admin.darjanafashion.com/assets/img/collection/${collection.collection_img || 'default.jpg'}`;
                    ...
                    <div class="swiper-slide">
                        <div class="shop-card style-2 wow fadeInUp" data-wow-delay="${delay}s">
                            ...
                            <div class=" text-center p-3">
                                <div class="row font-inst">
                                    <h7 class="title" style="font-weight: 300;">
                                        <a href="ProductDetails?id=${product.ids}">${displayName}</a>
                                    </h7>
                                </div>
                                <div class="row">
                                    <div class="price">
				...
				success: function(response) {
					if (response.data && response.data.length > 0) {
						...
						response.data.forEach(function(item, idx) {
							const imgUrl = `https://admin.darjanafashion.com/assets/img/sponsored/${item.brand_img || 'default.jpg'}`;
							const logoUrl = `https://admin.darjanafashion.com/assets/img/sponsored/logo/${item.brand_logo || 'default.png'}`;
        ...
        function renderTopSellingProducts() {
            ...
            window.topSellingProducts.forEach((product, index) => {
                ...
                //                 <div class="row font-inst">
                //                     <h7 class="title" style="font-weight: 300;">
                //                         <a href="ProductDetails?id=${product.product_id}">${displayName}</a>
                //                     </h7>
                //                 </div>
                //                 <div class="row">
                //                     <div class="price">
...
//                                     </div>
//                                 </div>
//                                 <p class="prose" style="text-align: justify;color: #343434">
...
//                                     <h6 class="mb-2">${getTranslation('other specifications (if any)')}</h6>
//                                     <textarea class="form-control customer-desc-input" rows="2" 
//                                         placeholder="Enter Specifications" 
//                                         maxlength="1000"></textarea>
//                                     <small class="text-muted">${getTranslation('max_1000_characters')}</small>
//                                 </div>
//                                 <div class="cart-btn">
...
//                                                 $button.prop('disabled', false).html('<i class="fas fa-heart me-2"></i>' + getTranslation('add_to_wishlist'));
...
//         }
        ...
        function reopenModalWithState(state) {
            // Load product details first (in case product data is not complete)
        ...
        //             const colorIndex = product.colors.findIndex(c => c === savedState.color);
        //             if (colorIndex >= 0) {
        ...
        //             const sizeIndex = product.sizes.findIndex(s => s === savedState.size);
        //             if (sizeIndex >= 0) {
        ...
        //         // Determine type text for notification
        //         const typeText = savedState.type === 'wishlist' ? 'wishlist' : 'cart';
                
        //         // Show a small notification that fields were restored (optional - can be removed)
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (data.status === "success" && data.path !== "") {
                        ...
                        let fullPath = "https://admin.darjanafashion.com/assets/videos/home/" + data.path;
