...
class ProductController
{
    ...
    function SQLArray()
    {
       ...
       $array[0] = "INSERT INTO product_details (
        ...
        $array[15] = "SELECT * FROM category_details where status='Active' AND ids != 0";
    ...
    function RequestAccept($FunctionEvents)
    {
        ...
        switch ($FunctionEvents) {
                        ...
                        if (!empty($this->productImages['name'][0]) && !empty($this->productImages['tmp_name'][0])) {
                            ...
                            foreach ($this->productImages['tmp_name'] as $key => $tmp_name) {
                                ...
                                if (move_uploaded_file($tmp_name, $original_path)) {
                                    ...
                                    // Store in database   // (product_id, product_image, image_original, image_product, image_thumb) 
                                                        // VALUES ('$product_id', '$filename', '$filename', '$filename', '$filename')";
            ...
            case 'get_product_details':
                ...
                $query = "SELECT * FROM product_details WHERE ids = '$productId' AND vendor_id = '" . $_SESSION["vendor_id"] . "'";
                ...
                if (!empty($productDetails)) {
                    ...
                    // Get product images
                    //$imageQuery = "SELECT ids, product_image FROM product_image WHERE product_id = '$productId'";
                ...
                if (!empty($_FILES['product_images']) && isset($_FILES['product_images']['tmp_name'][0]) && !empty($_FILES['product_images']['tmp_name'][0])) {
                    ...
                    foreach ($_FILES['product_images']['tmp_name'] as $index => $tmpName) {
                        if (!empty($tmpName) && $_FILES['product_images']['error'][$index] === 0) {
                            $imageId = $_POST['product_image_id'][$index] ?? null;
            ...
            case 'generate_encrypted_product_url':
                ...
                try {
                    ...
                    $checkSql = "SELECT ids FROM product_details WHERE ids = '$productId' AND vendor_id = '" . $_SESSION["vendor_id"] . "'";
