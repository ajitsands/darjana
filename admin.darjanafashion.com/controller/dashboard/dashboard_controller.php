...
class DashboardController
{
    ...
    function SQLArray()
    {
        ...
        // Dashboard Summary
        $array[0] = "SELECT 
            COUNT(DISTINCT CASE WHEN od.status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled') THEN od.ids END) as total_orders,
            ROUND(SUM(CASE WHEN od.status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled') 
                THEN (CASE WHEN od.discount_price = 0 THEN od.selling_price ELSE od.discount_price END * od.quantity) * (1 + od.tax_percentage / 100) 
                ELSE 0 END), 3) as total_sales,
            ROUND(SUM(CASE WHEN od.status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled') AND od.payment_status = 'PAID'
                THEN (CASE WHEN od.discount_price = 0 THEN od.selling_price ELSE od.discount_price END * od.quantity) * (1 + od.tax_percentage / 100) 
                ELSE 0 END), 3) as total_collected,
            (SELECT COUNT(DISTINCT customer_id) FROM order_details WHERE vendor_id = {$_SESSION['vendor_id']} AND status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled')) AS total_customers,
            (SELECT COUNT(ids) FROM product_details WHERE status = 'Active' AND vendor_id = {$_SESSION['vendor_id']}) as total_products
        FROM order_details od
        WHERE od.vendor_id = {$_SESSION['vendor_id']}";
        ...
        $array[2] = "SELECT 
                        DATE(od.placed_date) as date, 
                        COUNT(DISTINCT od.ids) as daily_orders,
                        ROUND(SUM((CASE WHEN od.discount_price = 0 THEN od.selling_price ELSE od.discount_price END * od.quantity) * (1 + od.tax_percentage / 100)), 3) as daily_sales
                    FROM order_details od
                    WHERE od.placed_date >= DATE_SUB(NOW(), INTERVAL ? DAY)
                      AND od.vendor_id = {$_SESSION['vendor_id']}
                      AND od.status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled')
                    GROUP BY DATE(od.placed_date)
                    ORDER BY DATE(od.placed_date)";
        
        $array[3] = "SELECT 
                        c.category_type as category,
                        COUNT(DISTINCT od.ids) as order_count,
                        ROUND(SUM((CASE WHEN od.discount_price = 0 THEN od.selling_price ELSE od.discount_price END * od.quantity) * (1 + od.tax_percentage / 100)), 3) as total_sales
                    FROM order_details od
                    JOIN product_details p ON od.product_id = p.ids
                    JOIN category_details c ON p.category_id = c.ids
                    WHERE od.placed_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)
        ...
        $array[6] = "SELECT 
                        street,
                        COUNT(DISTINCT customer_id) as customer_count,
                        COUNT(DISTINCT ids) as order_count,
                        ROUND(SUM((CASE WHEN discount_price = 0 THEN selling_price ELSE discount_price END * quantity) * (1 + tax_percentage / 100)), 3) as total_sales
                    FROM order_details
                    WHERE vendor_id = {$_SESSION['vendor_id']}
                      AND status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled')
                    GROUP BY street
                    ORDER BY total_sales DESC";
        ...
        switch ($FunctionEvents) {
            case 'get_dashboard_summary':
                ...
                $response = [
                    ...
                    'total_collected' => $currentData['total_collected'] ?? 0,
           ...
           case 'get_sales_data':
                ...
                //           GROUP BY DATE(placed_date)";
                 ...
                 $query = "SELECT 
                            DATE(placed_date) as sale_date, 
                            ROUND(SUM((CASE WHEN discount_price = 0 THEN selling_price ELSE discount_price END * quantity) * (1 + tax_percentage / 100)), 3) as daily_sales
                          FROM order_details
                          WHERE placed_date BETWEEN ? AND ?
                          AND vendor_id = {$_SESSION['vendor_id']}
                          AND status NOT IN ('Cancelled', 'product cancelled', 'Product Cancelled')
